...
export function activate(context: vscode.ExtensionContext) {
	...
	console.log('activating extension "starchat"');
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('starchat.explain', () => commandHandler('promptPrefix.explain')),
		vscode.commands.registerCommand('starchat.complete', () => commandHandler('promptPrefix.complete')),
		vscode.commands.registerCommand('starchat.refactor', () => commandHandler('promptPrefix.refactor')),
		vscode.commands.registerCommand('starchat.optimize', () => commandHandler('promptPrefix.optimize')),
		vscode.commands.registerCommand('starchat.clean', () => commandHandler('promptPrefix.clean')),
		vscode.commands.registerCommand('starchat.findProblems', () => commandHandler('promptPrefix.findProblems')),
		vscode.commands.registerCommand('starchat.documentation', () => commandHandler('promptPrefix.documentation')),
		vscode.commands.registerCommand('starchat.writeComments', () => commandHandler('promptPrefix.writeComments')),
		vscode.commands.registerCommand('starchat.writeTests', () => commandHandler('promptPrefix.writeTests')),
	);
...
class TextGenerationViewProvider implements vscode.WebviewViewProvider {
	public static readonly viewType = 'starchat.chatView';
	private _view?: vscode.WebviewView;
	private _starchatAPI?: HfInference;
	...
	// This private method initializes a new starchatAPI instance
	private _newAPI() {
		if (!this._authInfo || !this._settings?.apiUrl) {
			...
			this._starchatAPI = new HfInference(this._authInfo.apiKey || "xx");
		}
	}
	...
	public async search(prompt?: string) {
		...
		// Check if the starchatAPI instance is defined
		if (!this._starchatAPI) {
			this._newAPI();
		}

		// focus gpt activity from activity bar
		if (!this._view) {
			await vscode.commands.executeCommand('starchat.chatView.focus');
		} else {
		...
		if (!this._starchatAPI) {
			...
			const agent = this._starchatAPI;

			try {
				// HFInference
				let temp_response = "";
				for await (const output of this._starchatAPI.textGenerationStream({
	...
	private _getHtmlForWebview(webview: vscode.Webview) {
		...
		return `<!DOCTYPE html>
			...
			<body>
				<input class="h-10 w-full text-white bg-stone-700 p-4 text-sm" placeholder="Ask Starchat something" id="prompt-input"/>
